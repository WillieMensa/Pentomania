...
function languageButtons() {
	...
	document.getElementById('engButt').style.cssText = "top:" + (STAGE_OFFSET_Y + 1.6 * BLOCK_CELL_SIZE) + "px; left:" + (STAGE_X -3*BLOCK_CELL_SIZE) + "px; position: absolute;";
	document.getElementById('deuButt').style.cssText = "top:" + (STAGE_OFFSET_Y + 2.4 * BLOCK_CELL_SIZE) + "px; left:" + (STAGE_X -3*BLOCK_CELL_SIZE) + "px; position: absolute;";
}
...
{
	if (DEBUG)	{
		console.log('initLanguage con codigo: ' + langId );
	}
	sysLang = ((langId==null)? ( getStorage("elIdioma")	): langId);
